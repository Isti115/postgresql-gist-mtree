 ...
 *****************************************************************************/
  ...
  // PG_FUNCTION_INFO_V1(mus_gist_recv);
  //
  // Datum   // mus_gist_recv(PG_FUNCTION_ARGS)
  // {
  //     elog(NOTICE, "recv begin");
  //     StringInfo buf = (StringInfo) PG_GETARG_POINTER(0);
  //     mus_gist *result;
  // 
  //     // result = (mus_gist *) palloc(sizeof(mus_gist));
  //     // result->data = palloc(DATLEN + VARHDRSZ);
  //     // SET_VARSIZE(result->data, DATLEN + VARHDRSZ);
  //     // result->vl_dat[0] = pq_getmsgint64(buf);
  //     // result->data->vl_dat[0] = pq_getmsgbyte(buf);
  //     // elog(NOTICE, "recv, %c", result->data->vl_dat[0]);
  //     PG_RETURN_POINTER(result);
  // }
  // 
  // PG_FUNCTION_INFO_V1(mus_gist_send);
  // 
  // Datum   // mus_gist_send(PG_FUNCTION_ARGS)
  // {
  //     mus_gist *mus = PG_GETARG_MUS_GIST_P(0);
  //     StringInfoData buf;
  // 
  ...
  // 
  //     pq_begintypsend(&buf);
  //     // pq_sendint64(&buf, mus->vl_dat[0]);
  //     // pq_sendbyte(&buf, mus->data->vl_dat[0]);
...
mus_gist *mus_gist_deep_copy(mus_gist *source) {
  ...
  // SET_VARSIZE(destination, VARSIZE_ANY(source));
  // destination->...
  // strcpy(...);
...
int internal_mus_gist_distance(mus_gist *mus1, mus_gist *mus2) {
  // unsigned long length1 = VARSIZE_ANY(mus1) - OWNHDRSZ;
  // unsigned long length2 = VARSIZE_ANY(mus2) - OWNHDRSZ;

  char *data1 = mus1->vl_dat;
  char *data2 = mus2->vl_dat;

  // elog(NOTICE, "mus1: %lu | %s", length1, data1);
  // elog(NOTICE, "mus2: %lu | %s", length2, data2);
...
Datum mus_gist_consistent(PG_FUNCTION_ARGS) {
  ...
  // elog(NOTICE, "consistent | qcr: %d, kcr: %d, d: %d", query->coveringRadius, key->coveringRadius, d);
  ...
  if (GIST_LEAF(entry)) {
    *recheck = false;
    ...
    // if (d <= key->coveringRadius + query->coveringRadius) {
    //   retval = true;
  ...
  // elog(NOTICE, "#consistent end");
 
  // if (retval) {
  //   elog(
  //     NOTICE,
  //     GREEN("consistent | kcr: %d, d: %d | k: %s | rv: %d, rc: %d |"),
  //     key->coveringRadius,
  //     d,
  //     key->vl_dat,
  //     retval, *recheck
  ...
  //   elog(
  //     NOTICE,
  //     "consistent | kcr: %d, d: %d | k: %s | rv: %d, rc: %d |",
  //     // query->coveringRadius,
  //     key->coveringRadius,
  //     d,
  //     key->vl_dat,
  //     // query->vl_dat
...
Datum gist_union(PG_FUNCTION_ARGS) {
  // elog(WARNING, "gist_union start");
  // elog(NOTICE, "#union start");
  ...
  // elog(WARNING, "gist_union end");
  // elog(NOTICE, "#union end");
}
...
Datum mus_gist_penalty(PG_FUNCTION_ARGS) {
  ...
  *penalty = (newCoveringRadius < orig->coveringRadius ? 0 : newCoveringRadius - orig->coveringRadius);
  ...
  // *penalty = (
  //   newCoveringRadius < orig->coveringRadius ?
  //   orig->coveringRadius :
  //   newCoveringRadius - orig->coveringRadius
  // );

  // elog(WARNING, "gist_penalty end");
...
Datum gist_picksplit(PG_FUNCTION_ARGS) {
  ...
  switch (picksplitStrategy) {
    case PicksplitRandom:
      {
        // leftIndex = random() % maxoff;
        // rightIndex = random() % maxoff;
  ...
  // elog(NOTICE, "%d, %d", leftIndex, maxoff);
  ...
  // int real_index = raw_entryvec[0] - entryvec->vector;

  // elog(NOTICE, "picksplit_before");
  // mus_gist *ps_test = DatumGetMusGist(entryvec->vector[FirstOffsetNumber].key);
  // unionR = ps_test; 

  // if (ps_test->coveringRadius > 0) {
  //   elog(NOTICE, "picksplit_test: %d", ps_test->coveringRadius);
  // } 
  ...
  for (OffsetNumber i = FirstOffsetNumber; i <= maxoff; i = OffsetNumberNext(i)) {
    ...
    // int distanceL = internal_mus_gist_distance(unionL, current); 
    // int distanceR = internal_mus_gist_distance(unionR, current); 
  ...
  // elog(NOTICE, "picksplit_crL: %d", unionL->coveringRadius);
  // elog(NOTICE, "picksplit_crR: %d", unionR->coveringRadius);
